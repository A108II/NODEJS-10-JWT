...
const handleLogOut = async (req, res) => {
  ...
  if (!cookie?.jwt) {
    ...
    return res.sendStatus(204) // No content to send by response body
  }
  // If there is refresh token, compare refresh token with the refresh token in database
  ...
  const thisUser = { ...user_match, refreshToken: '' }
  user_db.setUsers([...otherUsers, thisUser]);
